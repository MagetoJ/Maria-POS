...
import DashboardView from '../components/PerfomanceDashboardView'; // <-- **CHANGED/ADDED LINE**
import { UtensilsCrossed, Building, Home, Settings, BarChart3 } from 'lucide-react'; // <-- **CHANGED/ADDED IMPORT**
...
// <-- **CHANGED LINE**: Added 'dashboard' to the activeView type
export default function POS({ isQuickAccess = false, onBackToLogin }: POSProps) {
  const { user } = useAuth();
  const [activeView, setActiveView] = useState<'menu' | 'tables' | 'rooms' | 'delivery' | 'dashboard'>('menu');
  ...
  // **NEW LINE**: Determine if the user should see the dashboard link
  ...
  const renderMainContent = () => {
    switch (activeView) {
      ...
      case 'dashboard': // <-- **CHANGED/ADDED CASE**
